...
if [ "$1" = "-h" ] || [ "$1" = "--help" ]; then
    ...
    echo ""
    echo "  # Authenticated crawl (cookies + headers)"
    echo "  $0 https://app.example.com \\"
    echo "      --auth-cookie \"session=abc123; token=xyz789\" \\"
    echo "      --auth-header \"Authorization: Bearer token123\""
    echo ""
    echo "  # SPA support (React, Vue, Angular apps)"
    echo "  $0 https://spa-app.com --wait-for-idle --page-wait 5"
    echo ""
    echo "  # With Semgrep for enhanced detection"
    echo "  $0 https://example.com --semgrep"
    echo ""
    echo "  # Full featured scan"
    echo "  $0 --semgrep --domains domains.txt --tabs 5 \\"
    echo "      --auth-cookie \"session=abc123\" --wait-for-idle \\"
    echo "      --discover-subdomains"
    echo ""
    echo "All ultimate_crawler.py options are supported:"
    echo "  --auth-cookie      Authentication cookies"
    echo "  --auth-header      Custom headers (can be used multiple times)"
    echo "  --wait-for-idle    Wait for network idle (SPAs)"
    echo "  --page-wait N      Wait N seconds after page load"
    echo "  --no-skip-media    Download all media files"
    echo "  --max-media-size   Max media file size (default: 10M)"
    echo ""
    exit 0
fi
